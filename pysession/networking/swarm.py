...
class Swarm():
    ...
    async def storage_servers_from_service_nodes(self) -> list:
        ...
        result = await request_jsonrpc(node_url, 'oxend_request', params, options={'snode': True})
        result = filter(lambda node: node.public_ip != '0.0.0.0', result)
        result = map(lambda node: f'https://{node.public_ip}:{node.storage_port}:/storage_rpc/v1')
        return result
    ...
    async def random_service_node(self, use_seed: bool = False) -> str:
        ...
        storage_nodes = map(lambda node: f'https://{node.ip}:{node.port}/storage_rpc/v1')
    ...
    async def get_swarm_node_url(self, public_key) -> str:
        ...
        if not self.swarm_map[public_key] or time.time() - self.swarm_map[public_key] > 3600:
            async with self.swarm_url_lock:
                node_url = await self.random_storage_node(False)
                node_data = await self.ublic_key_ask(node_url, 'get_snode_for_pubkey', public_key)
                if not node_data:
                    raise SwarmException(f'Could not get storage nodes from {node_url} pub: {public_key}')    
                if not 'snodes' in node_data:
                    raise SwarmException(f'Could not get storage nodes object from {node_url}, {public_key}')
